		...
		for (int i = 0; i < lable.length; i++) {
			for (int j = 0; j < lable[i].length; j++) {
				if (lable[i][j].isMineTag() == false) {
					...
					if (j > 0 && lable[i][j - 1].isMineTag() == true) {
						count++;
					}
					if (j < lable[i].length - 1 && lable[i][j + 1].isMineTag() == true) {
						count++;
					}
					if (i > 0 && lable[i - 1][j].isMineTag() == true) {
						count++;
					}
					if (i < lable.length - 1 && lable[i + 1][j].isMineTag() == true) {
						count++;
					}
